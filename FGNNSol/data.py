...
# 构建蛋白质图！！！！！   构建蛋白质图！！！！！ 构建蛋白质图！！！！！ 构建蛋白质图！！！！！ 构建蛋白质图！！！！！ 构建蛋白质图！！！！！
# 用于predict.py构建图数据
...
# 该函数将原始3D坐标转换为：节点特征: 综合局部几何环境（角度/距离/方向），边特征: 编码残基间的空间关系（距离/方向/相对旋转）
# 用于model.py的GPSoc类的向前传播中，第一步就是用来提取节点和边特征
def get_geo_feat(X, edge_index):
    ...
    return geo_node_feat, geo_edge_feat # 节点特征（geo_node_feat），边特征（geo_edge_feat）


# 下边的代码都是为get_geo_feat服务
